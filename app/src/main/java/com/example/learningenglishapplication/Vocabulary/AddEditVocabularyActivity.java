...
public class AddEditVocabularyActivity extends AppCompatActivity {
    ...
    private void loadVocabularyData() {
        if (vocabIdToEdit != -1) {
            Cursor cursor = vocabularyDAO.getVocabulary(vocabIdToEdit);
    ...
    private void saveVocabulary() {
        ...
        if (isEditing) {
            // Logic SỬA
            int rowsAffected = vocabularyDAO.updateVocabulary(vocabIdToEdit, word, meaning);
            if (rowsAffected > 0) {
            ...
            boolean isAdded = categoryDAO.addVocabulary(userId, categoryId, word, meaning);
            if (isAdded) {
